...
class DetectionConfig:
    ...
    # "helmet" and "no-helmet" (and "vest"/"no-vest") describe the same
    # physical region and are mutually exclusive - regular per-class NMS
    # doesn't suppress overlap ACROSS classes, so both can fire on the same
    # head/torso at once. Class-agnostic NMS keeps only the highest-
    # confidence box across classes when boxes overlap heavily.
